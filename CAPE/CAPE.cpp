...
CAPE::CAPE(int depth_height, int depth_width, int cell_width, int cell_height, bool cylinder_detection,
...
void CAPE::process(Eigen::MatrixXf &cloud_array, int &nr_planes_final, int &nr_cylinders_final, cv::Mat &seg_out,
                   vector<PlaneSeg> &plane_segments_final, vector<CylinderSeg> &cylinder_segments_final) {
    ...
    for (int cell_r = 0; cell_r < nr_vertical_cells; cell_r++) {
        for (int cell_c = 0; cell_c < nr_horizontal_cells; cell_c++) {
            ...
            if (Grid[stacked_cell_id]->planar) {
                cell_diameter = (cloud_array.block(stacked_cell_id * nr_pts_per_cell + nr_pts_per_cell - 1, 0, 1, 3) -
                                 cloud_array.block(stacked_cell_id * nr_pts_per_cell, 0, 1, 3)).norm();
                // Add truncated distance
                cell_distance_tols[stacked_cell_id] = pow(
                        min(max(cell_diameter * sin_cos_angle_4_merge, 20.0f), max_merge_dist), 2);
            }
    ...
    while (nr_remaining_planar_cells > 0) {
        ...
        RegionGrowing(nr_horizontal_cells, nr_vertical_cells, unassigned_mask, activation_map, Grid, cell_distance_tols,
                      x, y, seed_n, seed_d);
                    ...
                    if (new_ps.MSE < cy.MSEs[seg_id]) {
                        ...
                        for (int c = 0; c < nr_cells_activated; c++) {
                            if (cy.inliers[seg_id](c)) {
                                ...
                                grid_plane_seg_map.at<int>(cell_id / nr_horizontal_cells,
                        ...
                        for (int c = 0; c < nr_cells_activated; c++) {
                            if (cy.inliers[seg_id](c)) {
                                int cell_id = cy.local2global_map[c];
                                grid_cylinder_seg_map.at<int>(cell_id / nr_horizontal_cells,
    ...
    if (cylinder_detection) {
        ...
        for (int i = 0; i < nr_cylinders; i++) {
            ...
            grid_cylinder_seg_map_eroded.setTo((unsigned char) cylinder_code_offset + nr_cylinders_final,
                                               mask_eroded > 0);
        ...
        for (int cell_c = 0; cell_c < nr_horizontal_cells; cell_c++) {
            ...
            if (grid_plane_eroded_row_ptr[cell_c] > 0) {
                ...
                if (grid_cylinder_eroded_row_ptr[cell_c] > 0) {
                    // Set rectangle equal to assigned cell
                    seg_out(cv::Rect(c_offset, r_offset, cell_width, cell_height)).setTo(
                            grid_cylinder_eroded_row_ptr[cell_c]);
                } else {
                    // Set cell pixels one by one
                    stack_ptr = &seg_map_stacked[nr_pts_per_cell * cell_r * nr_horizontal_cells +
...
void
CAPE::RegionGrowing(unsigned short width, unsigned short height, bool *input, bool *output, vector<PlaneSeg *> &Grid,
                    vector<float> &cell_dist_tols, unsigned short x, unsigned short y, double *normal_1, double d) {
    int index = x + width * y;
    ...
    if (normal_1[0] * normal_2[0] + normal_1[1] * normal_2[1] + normal_1[2] * normal_2[2] < min_cos_angle_4_merge ||
        pow(normal_1[0] * m[0] + normal_1[1] * m[1] + normal_1[2] * m[2] + d, 2) >
        cell_dist_tols[index])//max_merge_dist
        return;
    output[index] = true;
